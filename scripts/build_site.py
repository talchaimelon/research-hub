...
def main():
    ...
    home += section('סגמנטים', 'עמודי עומק לכל קהל, עם הצעת ערך, MVP, אינטגרציות ו-skills.', f"<div class='grid'>{segment_cards}</div>")
    ...
    write(SRC / 'index.html', render_document('Market Research Hub', home, 'home', summary='מרכז אחד למחקרי שוק, קהלי יעד, המלצות מוצר והזדמנויות צמיחה.'))

    aud_cards = ''.join([card(p['name'], f"<strong>Pain:</strong> {p['pain_level']} | <strong>WTP:</strong> {p['wtp_score']} | <strong>מהירות החלטה:</strong> {p['decision_speed']}<br><br><strong>הצעת ערך:</strong> {p['recommended_offer']}<br><strong>MVP:</strong> {p['recommended_mvp']}", f"עדיפות #{p['priority_rank']}") for p in profiles])
    write(SRC / 'audiences.html', render_document('קהלי יעד', section('מפת קהלים', 'פחות טבלה כבדה, יותר snapshot ברור של מי שווה רדיפה.', f"<div class='grid'>{aud_cards}</div>") + section('השוואה מהירה', 'אם צריך מספרים, הם כאן.', table(['קהל','Pain','WTP','מהירות החלטה','קלות טירגוט','מורכבות טכנית'], [[p['name'], p['pain_level'], p['wtp_score'], p['decision_speed'], p['targeting_ease'], p['technical_complexity']] for p in profiles])), 'audiences', summary='השוואה ברורה בין ארבעת הסגמנטים המובילים, עם סדר עדיפויות ומסרי ליבה.'))

    research_cards = ''.join([card(r['title'], f"<strong>סגמנט:</strong> {r['segment']}<br><br>{r['summary']}<br><br><strong>כאבים:</strong> {r['pain_points']}<br><strong>התנגדויות:</strong> {r['objections']}", f"Score {r['priority_score']}") for r in research])
    write(SRC / 'research.html', render_document('מחקרי שוק', section('תובנות מחקר', 'כאן זה כבר knowledge base, לא רק dump של שורות.', f"<div class='grid'>{research_cards}</div>"), 'research', summary='מאגר מחקרי שוק חי, מסודר לפי סגמנטים, כאבים, התנגדויות וציון עדיפות.'))

    rec_cards = ''.join([card(r['title'], r['details'], f"{r['audience_name'] or 'כללי'} · {r['type']} · {r['priority']}") for r in recs])
    write(SRC / 'recommendations.html', render_document('המלצות', section('מה לעשות עכשיו', 'המלצות action-oriented לפי קהל, מוצר, מחקר ודפי נחיתה.', f"<div class='grid'>{rec_cards}</div>"), 'recommendations', summary='רשימת ההמלצות הפרקטיות ביותר, ממוסגרת לפי סוג ההחלטה שצריך לקחת.'))

    landing = ''.join([card(p['name'], f"<strong>מסר ליבה:</strong> {p['core_message']}<br><br><strong>זווית לדף נחיתה:</strong> {p['landing_angle']}", f"<a href='segments/{p['slug']}.html'>לעמוד הסגמנט</a>") for p in profiles])
    write(SRC / 'landing-pages.html', render_document('המלצות לדפי נחיתה', section('Angles לדפי נחיתה', 'איך למכור כל קהל בלי להיות כללי ומרוח.', f"<div class='grid'>{landing}</div>"), 'landing', summary='מסגור מסרים ו-headlines לדפי נחיתה לפי קהל, pain ו-outcome.'))
    ...
    opp_cards = ''.join([card(o['workflow'], f"<strong>סגמנט:</strong> {o['segment']}<br><strong>בעיה:</strong> {o['problem']}<br><strong>פתרון:</strong> {o['solution']}", o['priority']) for o in opportunities])
    write(SRC / 'opportunities.html', render_document('הזדמנויות מוצר', section('Opportunity Map', 'איפה הכי משתלם לבנות משהו narrow עם ROI חד.', f"<div class='grid'>{opp_cards}</div>"), 'opportunities', summary='מפת הזדמנויות למוצר, לפי workflow, pain ויכולת להגיע ל-ROI מהר.'))

    page_cards = ''.join([card(p['title'], p['summary'], f"{p['kind']} · {p['audience']}") for p in pages])
    write(SRC / 'pages.html', render_document('דפים ותוצרים', section('Library', 'כל הדפים, התוצרים והמסמכים שהופקו עד עכשיו.', f"<div class='grid'>{page_cards}</div>"), 'pages', summary='ספריית התוצרים של המחקר, כדי שתוכל לקפוץ מהר לכל asset רלוונטי.'))
    ...
    for p in profiles:
        ...
        body = section('למה הקהל הזה חשוב', 'Snapshot אסטרטגי, בלי לחפש בפסקאות.', f"<div class='grid-2'><div class='card'><div class='kicker'>מסר ליבה</div><h3>{p['core_message']}</h3><p class='muted'>{p['short_description']}</p></div><div class='card'><div class='kicker'>מה עושים next</div><h3>{p['next_step']}</h3><p class='muted'>{p['landing_angle']}</p></div></div>")
        ...
        body += section('מחקרי מפתח', 'התובנות העיקריות על הקהל הזה.', f"<div class='feature-list'>{''.join([f'<div class="feature-item"><strong>{r["title"]}</strong><br><span class="muted">{r["summary"]}</span></div>' for r in p_research])}</div>")
        body += section('המלצות', 'מה לעשות עם הקהל הזה בשלב הבא.', f"<div class='feature-list'>{''.join([f'<div class="feature-item"><strong>{r["title"]}</strong><br><span class="muted">{r["details"]}</span></div>' for r in p_recs])}</div>")
        ...
        write(SEGMENTS / f"{p['slug']}.html", render_document(p['name'], body, root='../', summary='עמוד עומק ממוקד לקהל אחד, עם value prop, MVP, אינטגרציות והצעדים הבאים.'))

    audience_master = section('Audience Master', 'מפת החלטה אחת שמרכזת את כל הקהלים במקום אחד.', f"<div class='grid'>{''.join([card(p['name'], f'<strong>מסר ליבה:</strong> {p["core_message"]}<br><br><strong>זווית לדף נחיתה:</strong> {p["landing_angle"]}<br><br><strong>צעד הבא:</strong> {p["next_step"]}', f'עדיפות #{p["priority_rank"]}') for p in profiles])}</div>")
    write(PAGES_DIR / 'audience-master.html', render_document('מסמך קהלי יעד מאוחד', audience_master, root='../', summary='עמוד־על שמרכז את מה שחשוב באמת: סדר עדיפויות, מסרים, MVP וצעדים הבאים.'))
